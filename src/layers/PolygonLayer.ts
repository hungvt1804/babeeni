...
export class PolygonLayer extends ImageLayer {
    ...
    constructor(p5: any, layerConfig: any, positionBase: any) {
        ...
        this.__initShapeGraphic();
        this.__initShapeNoFilled();
    ...
    protected __initShapeNoFilled() {
        this.shapeNoFilled = Support.getInstanceShapeNoFillGraphic(
            this.p5,
            this.p5.CENTER
        );
    }
    protected __initShapeGraphic() {
        this.shape = Support.getInstanceShapeGraphic(this.p5, this.p5.CENTER);
    }
    protected __initGraphic() {
        this.graphic = Support.getInstanceFirstGraphic(this.p5, this.p5.CENTER);
    }
    private _initgraphicSecond() {
        if (this.rotateAngle != 0) {
            this.graphicSecond = Support.getInstanceSecondGraphic(
                this.p5,
                this.p5.CENTER
            );
        }
    ...
    private _clearShapeNoFilled() {
        if (this.shapeNoFilled) {
            this.shapeNoFilled.clear();
        }
    }
    ...
    protected renderAngle(destImage: any) {
        ...
        let maxxRepeatX = Math.floor((BaseLayer.WIDTH * 2) / widthAfterScale);
        let maxxRepeatY = Math.floor((BaseLayer.HEIGHT * 2) / heightAfterScale);
